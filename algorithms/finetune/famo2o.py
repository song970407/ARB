...
class TrainConfig:
    ...
    balance_coef_min: float = 1.0
    balance_coef_max: float = 5.0
    sin_cos_skill_n: int = 10000
    sin_cos_skill_d: int = 6
    balance_policy_update_freq: int = 5
    ...
    sin_cos_skill_n: int = 10000
    sin_cos_skill_d: int = 6
    balance_policy_update_freq: int = 1
    expl_noise: float = 0.03  # Std of Gaussian exploration noise
    noise_clip: float = 0.5  # Range to clip noise
    iql_deterministic: bool = False  # Use deterministic actor
    normalize: bool = True  # Normalize states
    ...
    def __post_init__(self):
        ...
        if self.replay_buffer == "adaptive" or self.replay_buffer == "adaptive_traj":
            self.name = f"{self.name}-{self.env}-{self.seed}-{self.replay_buffer}-{self.weight_alpha}"
        else:
            self.name = f"{self.name}-{self.env}-{self.seed}-{self.replay_buffer}"
...
class ReplayBuffer:
    ...
    def update_sample_weight(self, universal_actor, balance_actor, config):
        pass
...
class ReplayBufferAdaptiveTrajectory(ReplayBuffer):
    ...
    def update_sample_weight(self, universal_actor, balance_actor, config):
        ...
        self._weight = np.zeros(self._buffer_size, dtype=np.float32)
        universal_actor.eval().cpu()
        balance_actor.eval().cpu()

        states = self._states[: self._size].detach().cpu()
        actions = self._actions[: self._size].detach().cpu()
        # beta = balance_actor.act(states, 'cpu')
        beta = balance_actor(states).mean.detach().numpy()

        beta_skill = beta_to_beta_skill_numpy(beta, config)
        state_cat = np.concatenate([states, beta_skill], axis=-1)
        state_cat = torch.from_numpy(state_cat).float()

        log_prob = torch.clamp(
            universal_actor.log_prob_standard(state_cat, actions) / self._action_dim,
            min=self._log_weight_min,
            max=self._log_weight_max,
        )
        ...
        print(episode_log_prob_list[-10:])"""
        self._weight[: self._size] = torch.exp(log_prob / self._weight_alpha).numpy()
        universal_actor.train().to(self._device)
        balance_actor.train().to(self._device)
...
def eval_actor(
    env: gym.Env,
    universal_actor: nn.Module,
    balance_actor: nn.Module,
    config: TrainConfig,
    n_episodes: int,
    seed: int,
    ...
    for _ in range(n_episodes):
        ...
        goal_achieved = False
        while not done:
            beta = balance_actor.act(state, device)
            beta_skill = beta_to_beta_skill_numpy(beta, config)
            state_cat = np.concatenate([state, beta_skill.flatten()])
            action = universal_actor.act(state_cat, device)
...
def beta_to_beta_skill_numpy(beta, config):
    beta = (config.balance_coef_max - config.balance_coef_min) * (
        beta + 1
    ) / 2 + config.balance_coef_min
    k = beta.reshape(-1, 1)
    i = np.arange(0, config.sin_cos_skill_d / 2, 1)

    inner_value = k / (config.sin_cos_skill_n ** (2 * i / config.sin_cos_skill_d))
    beta_skill = np.empty((k.shape[0], config.sin_cos_skill_d))
    beta_skill[:, 0 : config.sin_cos_skill_d : 2] = np.sin(inner_value)
    beta_skill[:, 1 : config.sin_cos_skill_d : 2] = np.cos(inner_value)
    return beta_skill
...
class FamO2O:
    def __init__(
        ...
        max_action: float,
        universal_actor: nn.Module,
        universal_actor_optimizer: torch.optim.Optimizer,
        balance_actor: nn.Module,
        balance_actor_optimizer: torch.optim.Optimizer,
        q_network: nn.Module,
        ...
        self.vf = v_network
        self.universal_actor = universal_actor
        self.balance_actor = balance_actor
        self.v_optimizer = v_optimizer
        self.q_optimizer = q_optimizer
        self.universal_actor_optimizer = universal_actor_optimizer
        self.balance_actor_optimizer = balance_actor_optimizer
        # self.actor_lr_schedule = CosineAnnealingLR(self.universal_actor_optimizer, max_steps)
        self.balance_coef_min = balance_coef_min
        self.balance_coef_max = balance_coef_max
        self.sin_cos_skill_n = sin_cos_skill_n
        self.sin_cos_skill_d = sin_cos_skill_d
        self.balance_policy_update_freq = balance_policy_update_freq
    ...
    def sin_cos_skill_func(self, balance_coef):
        k = balance_coef.reshape(-1, 1)
        i = torch.arange(0, self.sin_cos_skill_d / 2, 1).to(self.device)

        inner_value = k / (self.sin_cos_skill_n ** (2 * i / self.sin_cos_skill_d))
        skill = torch.empty((k.shape[0], self.sin_cos_skill_d)).to(self.device)
        skill[:, 0 : self.sin_cos_skill_d : 2] = torch.sin(inner_value)
        skill[:, 1 : self.sin_cos_skill_d : 2] = torch.cos(inner_value)
        return skill
    ...
    def _update_universal_policy(
        ...
        balance_coef = self.balance_actor(observations).rsample()
        balance_coef = (self.balance_coef_max - self.balance_coef_min) * (
            balance_coef + 1
        ) / 2 + self.balance_coef_min
        balance_skill = self.sin_cos_skill_func(balance_coef)
        universal_obs_cat = torch.cat([observations, balance_skill], dim=-1)
        policy_out = self.universal_actor(universal_obs_cat)
        ...
        log_dict["universal_actor_loss"] = policy_loss.item()
        self.universal_actor_optimizer.zero_grad()
        policy_loss.backward()
        self.universal_actor_optimizer.step()
        # self.actor_lr_schedule.step()

    def _update_balance_policy(self, observations: torch.Tensor, log_dict: Dict):
        balance_coef = self.balance_actor(observations).rsample()
        balance_coef = (self.balance_coef_max - self.balance_coef_min) * (
            balance_coef + 1
        ) / 2 + self.balance_coef_min
        balance_skill = self.sin_cos_skill_func(balance_coef)
        balance_obs_cat = torch.cat([observations, balance_skill], dim=-1)
        new_actions = self.universal_actor(balance_obs_cat).rsample()
        balance_policy_loss = (-1 * self.qf(observations, new_actions)).mean()
        log_dict["balance_actor_loss"] = balance_policy_loss.item()
        self.balance_actor_optimizer.zero_grad()
        balance_policy_loss.backward()
        self.balance_actor_optimizer.step()
    ...
    def train(self, batch: TensorBatch) -> Dict[str, float]:
        ...
        self._update_universal_policy(observations, actions, adv, log_dict)
        if self.total_it % self.balance_policy_update_freq == 0:
            self._update_balance_policy(observations, log_dict)
    ...
    def state_dict(self) -> Dict[str, Any]:
        return {
            ...
            "universal_actor": self.universal_actor.state_dict(),
            "universal_actor_optimizer": self.universal_actor_optimizer.state_dict(),
            "balance_actor": self.balance_actor.state_dict(),
            "balance_actor_optimizer": self.balance_actor_optimizer.state_dict(),
    ...
    def load_state_dict(self, state_dict: Dict[str, Any]):
        ...
        self.universal_actor.load_state_dict(state_dict["universal_actor"])
        self.universal_actor_optimizer.load_state_dict(
            state_dict["universal_actor_optimizer"]
        )
        self.balance_actor.load_state_dict(state_dict["balance_actor"])
        self.balance_actor_optimizer.load_state_dict(
            state_dict["balance_actor_optimizer"]
        )
...
def train(config: TrainConfig):
    ...
    universal_actor = GaussianPolicy(
        state_dim + config.sin_cos_skill_d,
        action_dim,
        max_action,
        dropout=config.actor_dropout,
    ).to(config.device)
    balance_actor = GaussianPolicy(
        state_dim, 1, max_action, dropout=config.actor_dropout
    ).to(config.device)
    v_optimizer = torch.optim.Adam(v_network.parameters(), lr=config.vf_lr)
    q_optimizer = torch.optim.Adam(q_network.parameters(), lr=config.qf_lr)
    actor_optimizer = torch.optim.Adam(actor.parameters(), lr=config.actor_lr)
    universal_actor_optimizer = torch.optim.Adam(
        universal_actor.parameters(), lr=config.actor_lr
    )
    balance_actor_optimizer = torch.optim.Adam(
        balance_actor.parameters(), lr=config.actor_lr
    )

    kwargs = {
        "max_action": max_action,
        "universal_actor": universal_actor,
        "universal_actor_optimizer": universal_actor_optimizer,
        "balance_actor": balance_actor,
        "balance_actor_optimizer": balance_actor_optimizer,
        "q_network": q_network,
        ...
        "balance_coef_min": config.balance_coef_min,
        "balance_coef_max": config.balance_coef_max,
        "sin_cos_skill_n": config.sin_cos_skill_n,
        "sin_cos_skill_d": config.sin_cos_skill_d,
        "balance_policy_update_freq": config.balance_policy_update_freq,
        "max_steps": config.offline_iterations,
    }

    print("---------------------------------------")
    print(f"Training FamO2O, Env: {config.env}, Seed: {seed}")
    print("---------------------------------------")

    # Initialize actor
    trainer = FamO2O(**kwargs)

    if config.load_model != "":
        policy_file = Path(config.load_model)
        trainer.load_state_dict(torch.load(policy_file, map_location=config.device))
        universal_actor = trainer.universal_actor
        balance_actor = trainer.balance_actor
    ...
    for t in range(int(config.online_iterations)):
        ...
        if t % config.model_update_freq == 0:
            for _ in range(config.model_update_freq):
                episode_step += 1
                beta = balance_actor.act(state, config.device)
                beta_skill = beta_to_beta_skill_numpy(beta, config)
                state_cat = np.concatenate([state, beta_skill.flatten()])
                action = universal_actor.act(state_cat, config.device)
        ...
        if t % config.weight_update_freq == 0:
            buffer.update_sample_weight(trainer.universal_actor, trainer.balance_actor, config)
        ...
        if (t + 1) % config.eval_freq == 0:
            ...
            eval_scores, success_rate = eval_actor(
                eval_env,
                trainer.universal_actor,
                trainer.balance_actor,
                config,
