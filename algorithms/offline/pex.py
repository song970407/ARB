...
def composite_actor(obs, qf1, qf2, actor_offline, actor_online, inverse_alpha, device):
    obs = torch.tensor(obs.reshape(1, -1), device=device, dtype=torch.float32)
    offline_action = actor_offline.act(obs, device=device)
    online_action = actor_online.act(obs, device=device)
    prob_offline = torch.min(
        qf1(obs, offline_action) * inverse_alpha,
        qf2(obs, offline_action) * inverse_alpha,
    ).exp()
    prob_online = torch.min(
        qf1(obs, online_action) * inverse_alpha, qf2(obs, online_action) * inverse_alpha
    ).exp()
...
def train(config: TrainConfig):
    ...
    for t in range(int(config.offline_iterations)):
        ...
        log_dict = trainer.train_offline(batch)
